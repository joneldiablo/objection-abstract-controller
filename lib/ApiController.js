...
var ApiController = function () {
  ...
  _createClass(ApiController, [{
    ...
    value: function get(req, res, next, modelObject, promise) {
      ...
      var page = req.query.page || Math.trunc(offset / limit) || 0;
      // fields or columns to return by row
      var fieldsReq = typeof req.query.fields === 'string';
      var queryByColumns = _typeof(req.query.filters) === 'object';
      var fields = new Set();

      if (fieldsReq) {
        fieldsReq = req.query.fields.split(',');
        fieldsReq.forEach(function (f) {
          return fields.add(f.trim());
        });
      }

      if (queryByColumns) {
        Object.keys(req.query.filters).forEach(function (f) {
          return fields.add(f);
        });
      }
      ...
      if (!modelObject) {
        ...
        var finalFields = Array.from(fields);
      ...
      if (queryByColumns) {
        modelObject.where(function (builder) {
          Object.keys(req.query.filters).forEach(function (col) {
      ...
      var response = modelObject.page(page, limit).then(function (resp) {
        return {
          total: resp.total,
          data: resp.results,
          limit: limit,
